  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden bg-black">
      ...
      <div className="relative z-10 min-h-screen flex flex-col">
        {/* Logo Section */}
        <header className="w-full pt-8 px-12">
        ...
        <main className="flex-1 relative">
          {/* Who is FGI - Top Left */}
          <motion.div
            className="absolute top-[12%] left-[5%] max-w-sm"
            initial={{ opacity: 0, x: -50 }}
            animate={animationStep >= 1 ? { opacity: 1, x: 0 } : {}}
            transition={{ duration: 0.5 }}
          >
            <div className="space-y-4">
              <h1 className="text-4xl font-light text-white">Who is FGI?</h1>
          ...
          <motion.div
            className="absolute bottom-[1rem] right-[5%] max-w-sm"
            initial={{ opacity: 0, x: 50 }}
          ...
          >
            <Card className="bg-black/40 border-forest/30 backdrop-blur-sm">
              <CardContent className="p-6 space-y-4">
                <h2 className="text-2xl font-light text-white">Our Platform</h2>
                ...
                <ul className="text-sm list-disc list-inside text-gray-200 space-y-2 font-light">
            ...
            >
              <Link href="/dashboard">
                <motion.div className="relative inline-block" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                  <Button
                    className="bg-gradient-to-r from-black to-forest hover:from-black/90 hover:to-forest/90 text-white px-8 py-6 text-xl rounded-full transition-all duration-300 shadow-lg hover:shadow-xl overflow-hidden group"
              ...
              <AnimatePresence>
                {showPopup && (
                  <motion.div
                    ...
                    className="absolute top-full left-1/2 transform -translate-x-1/2 mt-4 bg-black/80 text-white p-2 rounded shadow-lg backdrop-blur-sm border border-forest/30"
                  >
        ...
        {/* Strategic Partners */}
        <motion.footer
          className="w-full py-2 px-8 mt-auto"
          initial={{ opacity: 0, y: 50 }}
          animate={animationStep >= 3 ? { opacity: 1, y: 0 } : {}}
          transition={{ duration: 0.5 }}
        >
          <div className="max-w-7xl mx-auto">
            <h3 className="text-xl text-center mb-2 text-gray-300 font-light">Strategic Partners</h3>
            <div className="flex justify-center items-center gap-16">
              {/* UNIDO Logo */}
              <motion.div
                className="relative w-48 h-24 group"
